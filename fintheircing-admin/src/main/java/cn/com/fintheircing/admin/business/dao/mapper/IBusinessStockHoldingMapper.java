...
public interface IBusinessStockHoldingMapper {
    ...
    @Select("<script>select t1.stock_id as stockId,t2.stock_num as stockNo,t2.stock_name as stockName,SUM(t1.amount) as amount,Avg(t1.cost_price) as costPrice" +
            ...
            "from business_stock_holding t1 LEFT JOIN admin_transaction_summary t2 on t1.stock_id=t2.id where t1.contract_id=#{contractId} and t2.stock_num=#{stockNum} GROUP BY stock_num</script>")
